 ...
 * 权限管理 - 角色管理 
 * */
Ext.define('build.basic.RolePanel', {
	...
	requires: [
        ...
        'build.util.selector.MenuSelectorWindow'
    ],
	initComponent : function() {
		...
		this.rolePanel = Ext.create('Ext.form.Panel',{
			...
			border:false,
			layout:'column',
			defaults : {xtype:'panel',layout:'form',border:false,defaults:{margin : 2,anchor:'100%',xtype:'textfield'}},
		...
		//配置角色 window
		this.menuWin = Ext.create('build.util.selector.MenuSelectorWindow',{parentPanel:this});
			...
			Ext.Ajax.request({
				...
				success: function(resp) {
					...
					if (obj.result == 'success') {
						var msg = id == ""?'新建':'修改';
						App.getApplication().msg('提示', msg+'角色[ <font>'+roleName+'</font> ]成功！');
						this.rolePanel.reset();
						this.roleWindow.close();
					} else {
		...
		Ext.MessageBox.confirm('提示', '确定'+roleStr+' ？', function(btn) {
			...
			Ext.Ajax.request({
				...
				success: function(resp) {
					var obj = Ext.util.JSON.decode(resp.responseText);
					if (obj.result == 'success') {
						App.getApplication().msg('提示', roleStr+'成功!');
					} else {
	...
	configerMenu:function(value){
		var r = this.gridPanel.getSelectionModel().getSelection()[0];
		var s = this;
		Ext.MessageBox.confirm('提示', "确定给角色[<font color=red >"+r.get('roleName')+"</font>]配置如下菜单:</br><font color=red>"+
		this.menuWin.getNames()+"</font>?", function(btn) {
			...
			Ext.Ajax.request({
				...
				params: {roleId: r.get('id'),menuId:value==""? "":value},
				waitTitle: '请稍等片刻',
	       		waitMsg: '正在提交...',
				scope:this,
				success: function(resp) {
					var obj = Ext.util.JSON.decode(resp.responseText);
					if (obj.result == 'success') {
						App.getApplication().msg('提示', '配置菜单成功');
					} else {
						Ext.Msg.alert("提示", "错误信息:"+obj.info);
					}
					s.gridPanelLoadData();
	...
	gridPanelLoadData:function(){
		this.roleStore.load({params:{"start":0,"limit":25}});
	},
	/*
	 *  刷新表格
	 * */
